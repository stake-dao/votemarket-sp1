...
| Build-toolchain supply chain (rustup, sp1up, their downloads) | Host-side compromise, so key theft + selection, **not forgery** | The installer entry points are pinned: rustup-init comes from the immutable versioned archive checked against rustup's published sha256, sp1up from the commit the SP1 release tag resolves to, checked against a committed hash (both in `docker/Dockerfile`). Residual: the artifacts sp1up itself downloads (the cargo_prove tarball, and the succinct rust toolchain fetched inside cargo-prove) publish no upstream checksums and are unverified, and the `ubuntu:24.04` base image plus its apt packages still resolve by mutable tag at build time. A compromised host toolchain collapses to the host-machine row (key theft, censorship). It cannot forge on-chain values: the deployed verifier binds to `.vkey.prod` independently of anything the build container claims, so forgery still requires breaking SP1 soundness or the verifier owner key |
